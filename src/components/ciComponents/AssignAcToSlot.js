...
import ViewSlotsAssignsCI from '../ViewSlotsAssignsCI'
import { useSelector, useDispatch } from 'react-redux'
import { backendLink } from '../../keys'

export default function AssignAcToSlot() {
  ...
  const assignAcToSlot = async (event) => {
    event.preventDefault()
    event.stopPropagation()
    await axios({
      url: `${backendLink}/courseInstructor/assignAcademicMemberToSlot`,
      method: 'post',
        ...
        <Card className='ciCard' style={{ height: '33vw' }}>
          <Card.Body>
            ...
            <Button
              ...
              onClick={(event) => assignAcToSlot(event)}
            ...
            ) : message === 'staff does not teach this course' ? (
              <Form.Text
                className='messagesStyling'
                style={{ marginLeft: '22vw' }}
              >
                staff does not teach this course!
              </Form.Text>
            ) : (
              ''
            )}
